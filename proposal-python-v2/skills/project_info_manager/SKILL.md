...
description: |
  项目基本信息与团队职责维护技能（步骤1）。
  使用场景（穷举）：
  - 当用户询问项目基本信息时使用（触发词：查看项目、显示项目信息、项目基本信息、有哪些信息）
  - 当用户要修改产品编号或产品名称时使用（触发词：产品编号改成、修改产品名称、更新productNo、productName改成）
  - 当用户要管理团队成员时使用（触发词：添加成员、新增成员、加入成员、更新职责、勾选职责）
  - 当用户询问团队成员列表时使用（触发词：查看团队、显示团队成员、有哪些成员、团队职责）
  - 当用户进行日常对话时使用（触发词：你好、你是谁、介绍一下、帮助、能做什么、谢谢）
  注意：
  - 只有项目经理（isPM=true）可以修改数据，普通成员只能查看
  - 项目基本信息中只有productNo和productName可编辑，其他字段不可修改
license: MIT
