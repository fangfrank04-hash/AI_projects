...
# ============================================================================
# 快速评测 API（前端检索测试页面加按钮调用）
# ============================================================================

@evaluation_bp.route("/quick_eval", methods=["POST"])
async def quick_evaluation():
    """快速评测单个问题 — 前端检索测试页面点「评测」按钮调用。

    Body:
        question (str): 用户输入的问题（必填）
        kb_ids (list[str]): 知识库 ID 列表（必填，如 ["kb_abc123"]）
        tenant_id (str): 租户 ID（可选，后端自动从 session 获取）
        ground_truth (str): 标准答案（可选，用于计算 context_recall）
        chat_model_id (str): 指定的对话模型 ID（可选，不传则用默认）
        prompt_config (dict): 自定义 prompt 配置（可选）

    返回示例:
    {
        "code": 0,
        "data": {
            "question": "可以参与不良贷款转让业务的受让方机构主要有哪些",
            "answer": "根据相关法规，受让方机构主要包括...",
            "retrieved_chunks": [...],
            "faithfulness": 0.85,
    ...
    # ── 确保项目根目录在 Python path 中（用于导入 evaluation_wrapper） ──
    ...
    kb_ids = data.get("kb_ids", [])
    if not question:
        return _error("question 不能为空")
    if not kb_ids:
        return _error("kb_ids 不能为空（至少传一个知识库 ID）")

    # ── 获取租户 ID（优先从请求体，其次从登录态） ──
    ...
    if not tenant_id:
        return _error("无法获取 tenant_id，请登录或手动传入")
    ...
    try:
        ...
        return _error(f"导入 ragflow 核心模块失败: {e}", 500)

    # 构建 Dialog 对象（用 ragflow 内部的最小配置）
    try:
        if chat_model_id:
            # 用指定的模型
            ...
            kbs = await _thread_call(KnowledgebaseService.get_by_ids, kb_ids)
            if not kbs:
                return _error(f"知识库不存在: {kb_ids}")
            embd_id = kbs[0].embd_id
        else:
            # 用默认模型
            embd_config = await _thread_call(get_tenant_default_model_by_type, tenant_id, LLMType.EMBEDDING.value)
            embd_id = embd_config.get("llm_name", "") if embd_config else ""
            chat_config = await _thread_call(get_tenant_default_model_by_type, tenant_id, LLMType.CHAT.value)
            chat_model_id = chat_config.get("llm_name", "") if chat_config else ""
        ...
        return _error(f"构建 Dialog 配置失败: {e}", 500)
    ...
    reference = {}
    try:
        async for ans in async_chat(dia, messages, stream=False):
            if isinstance(ans, dict):
                answer = ans.get("answer", "")
                reference = ans.get("reference", {})
                break
    except Exception as e:
        return _error(f"RAG 调用失败: {e}", 500)

    if not answer:
        return _error("LLM 未返回有效答案")

    # ── 调用 eval_answer 计算四个指标 ──
    ...
    try:
        ...
        result_with_eval = eval_answer(ragflow_result, question=question, ground_truth=ground_truth)
        evaluation = result_with_eval.get("evaluation", {})

        # 检查是否有错误
        if "error" in evaluation:
            return _error(f"评测计算失败: {evaluation['error']}")
    except ImportError:
        return _error("evaluation_wrapper 模块未找到，请确认已放置到项目根目录", 500)
    except Exception as e:
        return _error(f"评测计算异常: {e}", 500)

    # ── 只返回评测结果 ──
    return _success({
        "question": question,
        "answer": answer[:500],  # 截断展示
        "retrieved_chunks": reference.get("chunks", []),
...
async def _thread_call(func, *args, **kwargs):
    """在独立线程中运行同步函数（避免阻塞异步事件循环）。"""
    from api.db.services.common_service import thread_pool_exec
    err, result = await thread_pool_exec(func, *args, **kwargs)
    if err:
