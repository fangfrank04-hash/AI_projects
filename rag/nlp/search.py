...
class Dealer:
    ...
    async def retrieval(
        ...
        # 调试日志：统计各文档的片段分布情况
        doc_chunk_stats = defaultdict(list)
        for i in valid_idx:
            id = sres.ids[i]
            chunk = sres.field[id]
            dnm = chunk.get("docnm_kwd", "")
            doc_chunk_stats[dnm].append({
                "id": id,
                "similarity": float(sim_np[i]),
                "rank": valid_idx.index(i) + 1
            })

        logging.debug(f"[检索统计] Top-K={top}, 符合条件的片段总数={filtered_count}")
        for dnm, chunks in sorted(doc_chunk_stats.items(), key=lambda x: len(x[1]) * -1)[:10]:
            ranks_list = [c["rank"] for c in chunks]
            min_rank, max_rank = min(ranks_list), max(ranks_list)
            logging.debug(f"  文档 '{dnm}': {len(chunks)} 个片段, 排名范围=[{min_rank}-{max_rank}]")
